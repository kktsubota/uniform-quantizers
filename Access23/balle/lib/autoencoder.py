    ...
    def build(self, input_shape):
        self._layers = [
            tfc.SignalConv2D(
                ...
                padding="same_zeros",
                use_bias=True,
                activation=tfc.GDN(name="gdn_0", data_format="channels_first"),
            ...
            tfc.SignalConv2D(
                ...
                padding="same_zeros",
                use_bias=True,
                activation=tfc.GDN(name="gdn_1", data_format="channels_first"),
                data_format="channels_first",
            ),
            tfc.SignalConv2D(
                self.num_filters,
    ...
    def build(self, input_shape):
        self._layers = [
            tfc.SignalConv2D(
                ...
                padding="same_zeros",
                use_bias=True,
                activation=tfc.GDN(name="igdn_0", inverse=True, data_format="channels_first"),
                data_format="channels_first",
            ),
            tfc.SignalConv2D(
                self.num_filters,
                ...
                padding="same_zeros",
                use_bias=True,
                activation=tfc.GDN(name="igdn_1", inverse=True, data_format="channels_first"),
                data_format="channels_first",
            ),
            tfc.SignalConv2D(
                3,
...
class _PCSJ(_Network):
    def __init__(self, config, quantize=True):
        super().__init__(config, quantize)
        C = self.num_chan_bn_including_heatmap if self.config.heatmap else self.config.num_chan_bn

        self.encoder = ThreeConvEncoder(arch_param_n, latent_size=C)
        self.decoder = ThreeConvDecoder(arch_param_n, latent_size=self.config.num_chan_bn)
